...
timer_node_t*
tw_add_timer(int time, handler_pt func, int threadid) {
...
void
tw_del_timer(timer_node_t *node) {
    node->cancel = 1;
}
...
gettime() {
	...
	t = (uint64_t)ti.tv_sec;
	t += ti.tv_nsec / 1000000000;
#else
	struct timeval tv;
...
timer_node_t*
tw_modify_timer(int time, timer_node_t* node, handler_pt func, int theradid) {
	tw_del_timer(node);
	return tw_add_timer(time, func, theradid);
}